...
void loop() {
  ...
  if (ldr < BAHAYA) {
    ...
    lcd.setCursor(0,0);  // Atur kursor ke kolom 0, baris 0 (baris atas LCD)
    lcd.print("API PADAM"); // Tampilkan pesan bahwa api sudah berhasil dipadamkan
    lcd.setCursor(0,1); // Atur kursor ke kolom 0, baris 1 (baris bawah LCD)
    lcd.print("Sistem normal");    // Tampilkan bahwa sistem kembali ke kondisi normal
